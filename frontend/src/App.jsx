...
function App() {
  ...
  async function loadDashboard(base = "/api/dashboard", silent = false, sc = activeScenario) {
  ...
  useEffect(() => { loadDashboard("/api/dashboard", false, activeScenario); return () => abortRef.current?.abort(); }, [activeScenario]);
  useEffect(() => { if (!autoRefresh || activeTimeline !== "now") return; const t = setInterval(() => loadDashboard("/api/dashboard", true, activeScenario), 5000); return () => clearInterval(t); }, [autoRefresh, activeScenario, activeTimeline]);
